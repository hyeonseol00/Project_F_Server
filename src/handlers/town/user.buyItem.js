...
const buyItemHandler = async (user, message) => {
  ...
  if (buyItem.itemCost === -1) {
    const response = createResponse('response', 'S_Chat', {
      playerId: user.playerId,
      chatMsg: ` ${buyItem.itemName}은 던전에서만 드랍 가능한 아이템입니다. `,
    });
    user.socket.write(response);
    return;
  }
  ...
  if (user.gold >= itemCost) {
    if (buyItem.itemType === 'potion') {
      ...
      const response = createResponse('response', 'S_Chat', {
        playerId: user.playerId,
        chatMsg: ` ${buyItem.itemName} 포션이 ${Number(quantity)}개 구매가 완료되었습니다. 골드가 ${user.gold} 남았습니다.`,
      });
      user.socket.write(response);
      return;
    } else {
      ...
      const response = createResponse('response', 'S_Chat', {
        playerId: user.playerId,
        chatMsg: ` ${buyItem.itemName} 아이템을 ${Number(quantity)}개 구매하였습니다. 남은 돈은 ${user.gold} 입니다. `,
      });
      user.socket.write(response);
      return;
    }
