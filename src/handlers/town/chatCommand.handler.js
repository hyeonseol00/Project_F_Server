...
export const buyItemHandler = (sender, message) => {
  
  console.log("BuyMessage",  message);
  const params = message.split(' ');
  console.log(`params ${params[0]}, ${params[1]}`);

  const response = createResponse('response', 'S_Chat', {
    playerId: sender.playerId,
    chatMsg: `[System] buyItemHandler`,
  });
  sender.socket.write(response);
  ...
  const buyResponse = createResponse('response', 'S_BuyItem', {
    item,
    gold: 12345
  });
  ...
  sender.socket.write(buyResponse);
  
};

export const sellItemHandler = (sender, message) => {
  console.log("SellMessage",  message);
  const params = message.split(' ');
  console.log(`params ${params[0]}, ${params[1]}`);

  // const response = createResponse('response', 'S_Chat', {
  //   playerId: sender.playerId,
  //   chatMsg: `[System] sellItemHandler`,
  // });
  // sender.socket.write(response);
  ...
  const sellResponse = createResponse('response', 'S_SellItem', {
    item,
    gold: 54321
  });
  ...
  sender.socket.write(sellResponse);
};
